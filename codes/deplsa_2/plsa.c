...
void iniStructures(Mat *pwd, Mat *pwz){
	...
	int D=pwd->rows, W=pwd->cols, Z=pwz->rows;
...
void Mstep(Mat *pwd, Mat *pwz, float lambda){

	int D=pzwd->size[1]; // numero de documentos

	// TBB parallelisation
	parallel_for_(Range(0, D), Parallel_Mstep_pzd(pwd,pzwd,lambda,pzd));
}
...
void saveParameters(char *outName, int app){
	...
	freeStructures(); // liberamos el espacio de las matrices creadas en 'iniStructures'
}
