...
//  ncols -> numero de columnas de la matriz de entrada
// OUT:
...
Mat* subMatRows(Mat* matIn, int row_ini, int row_fin){
	...
	int new_nrows = row_fin - row_ini + 1;
	
	// Matriz de salida en formato OpenCV 
	Mat *matOut = new Mat(new_nrows, matIn->cols, CV_32FC1, Scalar::all(0));

	// Recorremos la matriz
	for(int r=0;r<new_nrows;r++)
		for(int c=0;c<matIn->cols;c++)
			matOut->at<float>(r,c) = matIn->at<float>(r+row_ini,c);
